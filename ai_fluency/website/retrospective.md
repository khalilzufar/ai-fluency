# Launch retrospective

## What worked

- A single public-safe case study made the site coherent.
- A static host kept deploys simple and removed runtime infrastructure.
- The interactive case explorer demonstrates usefulness without hiding the evidence.

## What changed during build

- The design shifted from a generic project grid to a research-notebook layout with an evidence ribbon.
- The agent was reduced to three read-only tools so its behavior stays explainable.
- External analytics and other optional distribution steps were kept explicit instead of being implied.

## Next iteration

Collect independent human critique, add a real privacy-reviewed analytics property if needed, and publish a follow-up case when there is a new decision worth documenting.
